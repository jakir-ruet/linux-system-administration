...
```bash
wsl --list --verbose
wsl --import RHEL C:\WSL\RHEL C:\Users\Jakir\Downloads\Compressed\rhel-10.0-x86_64-wsl2.tar.gz --version 2
mkdir C:\WSL\RHEL
wsl -d RHEL
sudo subscription-manager register
sudo dnf update -y
wsl --set-default RHEL # Set RedHat as the default WSL distro
```
...
wsl --list --verbose
mkdir C:\WSL\Ubuntu
wsl --install -d Ubuntu-22.04 # Install from Microsoft Store or WSL directly
# Manual
wsl --import Ubuntu-22.04 C:\WSL\Ubuntu C:\Users\Jakir\Downloads\Compressed\ubuntu-22.04-x86_64-wsl2.tar.gz --version 2
wsl -d Ubuntu-22.04
sudo apt update && sudo apt upgrade -y
sudo apt install -y build-essential curl wget git vim unzip htop net-tools
wsl --set-default Ubuntu-22.04 # Set Ubuntu as the default WSL distro
```
...
| Destination   | Gateway                  | Genmask       | Flags | Interface       | Meaning                                                                        |
| ------------- | ------------------------ | ------------- | ----- | --------------- | ------------------------------------------------------------------------------ |
| default       | UBUNTU-SERVER.mshome.net | 0.0.0.0       | UG    | eth0            | Default route → sends all unknown traffic to the main router (internet access) |
| 172.17.0.0    | 0.0.0.0                  | 255.255.0.0   | U     | docker0         | Docker internal network for containers                                         |
| 172.24.16.0   | 0.0.0.0                  | 255.255.240.0 | U     | eth0            | Local network → direct communication (no gateway needed)                       |
| 192.168.49.0  | 0.0.0.0                  | 255.255.255.0 | U     | br-bef0663229f3 | Custom Docker bridge network                                                   |
| 192.168.122.0 | 0.0.0.0                  | 255.255.255.0 | U     | virbr0          | Virtual machine network (libvirt/KVM)                                          |

![Kernel IP routing table](/img/kernel-IP-routing-table.png)
...
```bash
sudo nmcli con mod "Wired connection 1" ipv4.addresses 192.168.1.130/24
