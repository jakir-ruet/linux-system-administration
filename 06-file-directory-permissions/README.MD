...
SUID is a special permission for executable files that allows the file to be executed with the privileges of the file owner, rather than the user who runs it.

Make it executable and owned by root, in `/home` directory

```bash
sudo chmod +x suid-demo.sh
sudo chown root:root suid-demo.sh
ls -la
-rwxr-xr-x  1 root  root    31 Aug  1 03:17 suid-demo.sh
```

Set the SUID bit (octal 4), in `/home` directory

```bash
sudo chmod 4755 suid-demo.sh
ls -la
-rwsr-xr-x  1 root  root    48 Aug  1 03:24 suid-demo.sh # The `s` in `rws` indicates SUID is active with red mark.
```
...
```bash
./suid-demo.sh
This script runs as: jakir # should see
```

`NB:` `Success!` The script runs with `root privileges` due to SUID.

##### Set Group ID - SGID

SGID is a special permission that behaves differently on files and directories.

- `On files:` Runs the file with the group ID of the file, not the user's group.
- `On directories:` Makes all new files/directories inside inherit the group of the parent directory.

Create a shared group and directory (e.g dev team)

```bash
sudo groupadd devgroup
sudo mkdir devteam # from `/home`
sudo chown root:devgroup devteam # from `/home`
ls -la
```

Set SGID bit (octal 2) and permissions

```bash
sudo chmod 2775 devteam
ls -la
drwxrwsr-x  2 root  devgroup 4096 Aug  1 03:35 devteam
```

Add yourself to the group

```bash
sudo usermod -aG devgroup jakir # $USER - jakir
newgrp devgroup   # Apply group immediately without logout
cd devteam/
touch file1.txt
ls -l file1.txt
-rw-rw-r-- 1 jakir devgroup 0 Aug  1 03:39 file1.txt
```

`NB:` File inherits the `devgroup` group, even if your default group is different.

##### Sticky Bit

Sticky Bit is a permission that allows only the file's owner or root to delete or rename files within a world-writable directory.

Create a public directory and verify

```bash
sudo mkdir /tmp/shared-folder
sudo chmod 1777 /tmp/shared-folder
ls -ld /tmp/shared-folder
drwxrwxrwt 2 root root ... /tmp/shared-folder
```

`NB:` The `t` in the end = Sticky Bit is set.

Create a file inside

```bash
cd /tmp/shared-folder
echo "test" > myfile.txt
```

`NB:` Only `file owner` or `root` can delete the file — even in world-writable directory.

###### Summary

| **Feature** | **SUID**                              | **SGID**                              | **Sticky**                    |
| ----------- | ------------------------------------- | ------------------------------------- | ----------------------------- |
| Applies To  | Files only                            | Files & Dirs                          | Directories                   |
| Octal       | 4                                     | 2                                     | 1                             |
| Symbol      | s                                     | s                                     | t                             |
| Legend      | Special permission + execute - `4775` | Special permission + execute - `2775` | Sticky bit + execute - `1775` |
| Effect      | Owner's                               | Group's                               | No effect on files            |
| Example     | `/usr/bin/passwd`                     | `srv/devteam/` shared folder          | `/tmp` directory              |
